...
Shader "AC/Time Of Day System Free/Skybox/Procedural"  
{
...
//				for(int i=0; i<int(kSamples); i++) // Loop removed because we kept hitting SM2.0 temp variable limits. Doesn't affect the image too much.
			...
			OUT.skyColor += (saturate(1-dot(eyeRay.y, 0.5)) * _NightColor.rgb) * 0.10;
		...
		half4 frag (v2f IN) : SV_Target
		{
			...
			half  moonBackgroundMask   = (1.0 - moonColor.a);
			moonColor  *= 1.0 - moonBackgroundMask;

			//IN.skyColor   +=moonBackgroundMask; // debug.
			...
			#if defined(MOON) 
			starsColor.rgb  *= (unity_ColorSpaceDouble.rgb) *  moonBackgroundMask;
			#else
			...
			//IN.skyColor     += lerp(1, 2 * starsNoise.rgb, _StarsTwinkle); // debug.
